...
def generate_patch(filepaths, out):
    ...
    f_meta = open('patch/file_metas.txt', 'w', encoding='utf8')
    ...
    for i in range(len(out)):
        ...
        f_ori = open('patch/' + filenames[i] + '.old', 'rb')
        f_new = open('patch/' + filenames[i] + '.new', 'rb')
        ...
        while True:
            ...
            if old != new:
                diff_cnt = diff_cnt + 1
                po = open('patch/' + out[i] + '_o/' + str(addr) + '.patch', 'wb')
                po.write(old)
                po.close()
                no = open('patch/' + out[i] + '_n/' + str(addr) + '.patch', 'wb')
                no.write(new)
                no.close()
